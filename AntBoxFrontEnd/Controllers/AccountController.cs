...
namespace AntBoxFrontEnd.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            _Repository = new UserRepository(new AntBoxContext());
        }
        ...
        public ActionResult AntBoxUpdateAccountAjax(string name, string lastname, string lastname2, string mail, string phone, string cellphone, string password)
        {
            ...
            if (res)
            {
                ...
                var resp = _Repository.UpdateUser(us);
        ...
        [HttpPost]
        [AllowAnonymous]
        public async Task<ActionResult> AntBoxRegisterAjax(string email, string name, string lastname, string password, string username, string verifoption, string phone)
        {
        ...
        public ActionResult AntBoxLoginAjax(string username, string password)
        {
            ...
            if (customer != null)
            {
                Users us = new Users { email = customer.Email, customerid = customer.Id };
                us = _Repository.GetUser(us);
                if (us != null)
                {
                    if (Convert.ToBoolean(us.status))
                    {
                        Session["customer"] = customer;
                        return PartialView("_LoginPartial");
                    }
                    ...
                    {
                        return Json(new { success = false, user = customer, message = "Debe validar su cuenta, por favor verifique su correo electrónico" }, JsonRequestBehavior.AllowGet);
                    }
                }
                else
                {
        ...
        [AllowAnonymous]
        public async Task<ActionResult> ConfirmationEmail(string confirmation_token)
        {
            if (!String.IsNullOrEmpty(confirmation_token))
            {
                var vs = _Repository.ValidateUser(confirmation_token);
                if (vs)
                {
                    TempData["IndValidate"] = true; TempData["message"] = "Verificación exitosa";
                }
                else
                {
                    var us = _Repository.GetUserByToken(confirmation_token);
                    if (us != null)
                    {
                        us.token = Code.HashHelper.GenerateToken();
                        _Repository.Save();
                        await SendEmail("ConfirmationEmail", "Account", us, "ConfirmationEmail", "Confirma tu cuenta");
                    }
                    TempData["IndValidate"] = false; TempData["message"] = "Token expirado";
                }
            }
            else
            {
        ...
        private async Task SendEmail(string actionName, string controllerName, Users User, string emailTemplate, string emailSubject)
        {
            var callbackUrl = Url.Action(actionName, controllerName, new { confirmation_token = User.token }, protocol: Request.Url.Scheme);
            var message = await EMailTemplate(emailTemplate);
            message = message.Replace("@ViewBag.Link", callbackUrl);
            await MessageServices.SendEmailAsync(User.email, emailSubject, message);
        }
